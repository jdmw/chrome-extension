...
var htmlToMarkdown = function(){
  ...
  var getCode = function(e){
    ...
    var lang = location.pathname.substring(1,location.pathname.indexOf('/',1))
    return '\n```' + lang + '\n' + $ncode.innerText + "\n```\n"
  }
  ...
  for ( var e of document.querySelector('.article-intro').children){
    nodeName = e.nodeName
    // debugger
    if( nodeName.search('^H\\d$') == 0){ // headers
      md += "\n" + parseHeader(e) + "\n\n"
    }else if( nodeName === 'PRE'){ // block text
      md += '\n```text\n' + e.innerText + "\n```\n"
    }else if(e.classList.contains('example')){ // codes
      md += getCode(e) + "\n"
