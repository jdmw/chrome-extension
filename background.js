...
var match = (function(matcher,url){
  ...
  return function(matcher,urlinfo){
    ...
    for ( var k in matcher){
      if( k.startsWith('url')){
        switch(k){
          case "urlContains" : if (urlinfo.url.search(matcher[k]) < 0) return false ;break;
        }
      }else{
        if( !urlinfo['host'] ) {
...
var getLoginScript = function(user){
  ...
  if ( !script ){
    ...
    if(pageAction['loginMethodLable']){
      script += clickBtnTpl.replace('{selector}',pageAction['loginMethodLable'])
    }
    if(pageAction['userfield']){
      script += setValTpl.replace('{selector}',pageAction['userfield']).replace('{value}',account['username'])
    }
    script += setValTpl.replace('{selector}',pageAction['passwordField'] || "[type=password]").replace('{value}',account['password'])
    script += clickBtnTpl.replace('{selector}',pageAction['submitButton'] || '[type=submit]')
